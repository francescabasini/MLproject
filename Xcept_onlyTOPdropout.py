...
###   Xception training only top with a Dropout(0.3) layer   ###
...
# save model
model.save(os.path.join(work_dir, 'Xcept_onlyTOPdropout_10epoch.h5'))
model.save_weights(os.path.join(work_dir, 'Xcept_onlyTOPdropout_10epoch_WEIGHTS.h5'))

# save history in a pickle file
with open(os.path.join(work_dir, 'hist_Xcept_onlyTOPdropout_10epoch'), 'wb') as file_pi:
...
# model = load_model(os.path.join(work_dir, 'Xcept_onlyTOPdropout_10epoch.h5'),
...
model.fit_generator(generator=train_generator,
                    ...
                    callbacks=[learning_rate_reduction])
# save model
model.save(os.path.join(work_dir, 'Xcept_onlyTOPdropout_20epoch.h5'))

# load the history of the saved model from the pickle file
with open(os.path.join(work_dir, 'hist_Xcept_onlyTOPdropout_10epoch'), 'rb') as file_pi:
...
with open(os.path.join(work_dir, 'hist_Xcept_onlyTOPdropout_20epoch'), 'wb') as file_pi:
...
best_model = load_model(os.path.join(work_dir, 'Xcept_onlyTOPdropout_10epoch.h5'),
                         custom_objects={'top_3_categorical_accuracy': top_3_categorical_accuracy,
                                         'precision': precision,
                                         'recall': recall,
                                         'F1_score': F1_score})
