...
# build model
model_finetune = load.model('data/ResNet18_RNrandTOP.h5') #loading net with TOP only trained for 5 epochs
...
# training
model_finetune.compile(optimizer='Adam', loss='categorical_crossentropy',\
              metrics=['accuracy', top_1_categorical_accuracy, top_3_categorical_accuracy, precision, recall, F1_score])
model_finetune.fit_generator(generator=train_generator,\
                    steps_per_epoch=STEP_SIZE_TRAIN,\
                    validation_data=valid_generator,\
                    validation_steps=STEP_SIZE_VALID,\
                    epochs=8, verbose=2,callbacks=[learning_rate_reduction])
# Remaning number of epochs in Visnawathan
model.save('data/ResNet18_Finetune_8epochs.h5') 

# list all data in history
print(model_finetune.history.history.keys())
# summarize history for accuracy
plt.plot(model_finetune.history.history['acc'])
plt.plot(model_finetune.history.history['val_acc'])
...
model_finetune.save('data/ResNet18_Finetune_25epochs.h5') 

#HOW TO LOAD DATA SAVED FOR 5 EPOCHS?
# list all data in history
