...
model50drop.save('data/ResNet50_dropTOP_11epochs.h5')

# accuracy plots
import pickle
import os
os.system("mkdir dictionaries")
with open('dictionaries/ResNet50_dropTOP_11epochsDict.pkl', 'wb') as file_pi:
...
plt.savefig('ResNet50_dropTOP_11epochs.png')
...
preds = model50drop.predict_generator(generator=best_valid_generator, steps=len(best_valid_generator), verbose=1)
...
with open('dictionaries/ResNet50_dropTOP_11epochsDict.pkl', 'rb') as file_pi:
    hist = pickle.load(file_pi)

#Writing metrics results in a txt file
acc_train = hist['acc'][9]
print('Training accuracy of best model is: {}'.format(acc_train))
evalu = model50drop.evaluate_generator(generator=valid_generator, steps=len(valid_generator))
acc_valid = evalu[1]
...
file = open("ResNet50_dropTOP_11epochs_metrics.txt", "w") 
...
plt.savefig('ResNet50_dropTOP_11epochs_ConfusionMatrix.png')
